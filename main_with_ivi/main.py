...
async def charge_five_dollars(call: CallbackQuery, state: FSMContext):
	await state.finish()
	await bot.delete_message(call.message.chat.id, call.message.message_id)
	await bot.send_message(call.message.chat.id, "<b>Введите данные карты в формате:</b>\n<i>[Номер карты]|[Месяц]|[Год]|[CVV]</i>\n<b>Пример:</b>\n<i>4890494678599200|12|24|711</i>\n\n<b>Не рекомендуется вводить слишком много карт, я пока не сделал разбивку сообщения на несколько, так что можно словить ограничение от тг на кол-во символов.</b>", reply_markup = await menu.return_all())
	await CardEnterDollar.card.set()


@dp.callback_query_handler(text="charge_one_rub", state="*")
async def charge_one_rub_ivi(call: CallbackQuery, state: FSMContext):
	await state.finish()
	await bot.delete_message(call.message.chat.id, call.message.message_id)
	await bot.send_message(call.message.chat.id, "<b>Введите данные карты в формате:</b>\n<i>[Номер карты]|[Месяц]|[Год]|[CVV]</i>\n<b>Пример:</b>\n<i>4890494678599200|12|24|711</i>\n\n<b>Не рекомендуется вводить слишком много карт, я пока не сделал разбивку сообщения на несколько, так что можно словить ограничение от тг на кол-во символов.", reply_markup = await menu.return_all())
	await CardEnterIvi.card.set()
...
@dp.message_handler(IsPrivate(), state=CardEnterIvi.card)
async def accept_card_ivi(message: types.Message, state: FSMContext):
	await message.answer("<b>❇️ Карты приняты. Проверка может занять несколько минут (в зависимости от кол-ва карт).</b>")
	msg = message.text
	formatted = msg.replace("|", " ")
	cards = formatted.split()
	...
	for card in res_cards:
		...
		data = {
			"email": "forsteamspace@gmail.com",
			"password": "TF98CAB3954",
			"device": "Windows Chrome v.95.0.4638.69 aeec0",
			"app_version": "870",
			"session": "6fe2ff688563089417920809_1651525485-0irnP3PL9pshAeuextiLqiQ",
			"session_data": "eyJ1aWQiOjg1NjMwODk0MTc5MjA4MDl9.YX8Kfg.FlBBtJ6UakfUoAU870MnaCBcOk4",
		}


		async with aiohttp.ClientSession() as session:
			async with session.post(url="https://api.ivi.ru/mobileapi/user/login/ivi/v5/", data=data, headers=headers) as resp:
				check = await resp.json()
				verdikt = check["message"]
